...
var _pay = function (
  ...
  // passed from WEB
  var pNoti = "";
  if (authResult.P_NOTI) pNoti = authResult.P_NOTI;
  ...
  var exeShopLogic = function (paymentResult) {
    ...
    try {
      ...
      if (shopResult.success) defer.resolve();
      else defer.reject(shopResult.message);
    } catch (e) {
      defer.reject(e);
    }
  ...
  inipayUtil.sendPostRequest(
    authResult.P_REQ_URL,
    authPostData,
    function (paymentRes) {
      var paymentResult = "";
      ...
      paymentRes.on("end", function () {
        ...
        if (paymentResult.P_STATUS !== "00") {
          ...
          failCallback(
            "[ " + paymentResult.P_STATUS + "] " + paymentResult.P_RMESG1
          );
        }
...
// bank transfer
var _noticeBank = function (notiData, res, successCallback) {
  var validationGuide = {
    P_STATUS: "any",
    P_RMESG1: "string",
    P_TID: "string",
...
var noticeBank = function (notiData, res, successCallback, failCallback) {
  try {
    if (typeof successCallback !== "function")
      successCallback = function () {
        return true;
      };
    return _noticeBank(notiData, res, successCallback);
  } catch (e) {
    failCallback(e);
  }
};
